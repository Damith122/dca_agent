...
MAX_DCA_STEPS = min(2, max(0, _env_int("MAX_DCA_STEPS", 1)))        # reduced from 3 (2026-08 $33-account risk fix) - 3 steps required 112% of a $33 account's margin to fully cascade and produced ~45% worst-case single-trade loss; 2 steps fits within the account (~62.5% margin) and cuts worst-case loss to ~25% while keeping the final-DCA low-probability-recovery gate active at the depth where trade evidence showed it protecting against genuine trend moves
...
# --- Entry Engine V2 ---------------------------------------------------------
ENTRY_SCORE_THRESHOLD = min(0.95, max(0.70, _env_float("ENTRY_SCORE_THRESHOLD", 0.75)))  # reject weak entries even if Railway env is set too low
